...
load("~/Desktop/research/fBARTselect_repo/analysis_files/data/mice_AJ_FJ_union/AJ-FJ_union.Rda")
combined_list = lapply(combined_list, function(x) x[, !(names(x) %in% "week")])
names(combined_list) = c("x1", "x2", "x3", "x4", "x5")
radiation = substring(colnames(combined_list[[1]]), 2, 2)
outcome = ifelse(radiation == "0", 0, 1)
time_points = 1:nrow(combined_list[[1]])

# impute missing values with interpolation 
combined_list_impute = lapply(combined_list, function(df) {
...
for (i in 1:length(unique(fold_vec))) {
  ...
  train_list = purrr::map(combined_list_impute, ~ select(.x, which(keep_cols)))
  outcome_train = ifelse(substring(colnames(train_list[[1]]), 2, 2) == "0", 0, 1)
  
  keep_cols = fold_vec == i
  test_list = purrr::map(combined_list_impute, ~ select(.x, which(keep_cols)))
